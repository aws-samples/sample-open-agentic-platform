...
| langfuse ApplicationSet "map has no entry for key \"hub_cluster_name\"" (blocks all agentic addons — parent `agent-platform-addons` app retries forever and never applies later sync-wave ApplicationSets), OR langfuse-spoke "could not get secret data" / ExternalSecret reads `spoke-*/keycloak-clients` | langfuse `secretManagerKey` resolved to the wrong/empty annotation; the hub and spoke cluster secrets carry different annotations | The langfuse OIDC client is a **hub singleton** (one Keycloak `langfuse` client in the shared `platform` realm, hub-domain redirect), so **every** cluster's langfuse must read the hub's `<hub>/keycloak-clients` secret. Neither annotation alone covers both cluster types: the **hub** secret (ESO from Secrets Manager `<hub>/config` `.metadata`) has `aws_cluster_name=<hub>` but **not** `hub_cluster_name`; the **spoke** secrets (fleet-secret Helm chart) have `aws_cluster_name=<spoke>` **and** `hub_cluster_name=<hub>`. So `secretManagerKey` in `gitops/addons/bootstrap/default/addons.yaml` uses a default expression — `{{default (index .metadata.annotations "aws_cluster_name") (index .metadata.annotations "hub_cluster_name")}}/keycloak-clients` — rendering `<hub>/keycloak-clients` everywhere: hub falls back to `aws_cluster_name=<hub>`; spokes use `hub_cluster_name=<hub>`. There is no per-spoke langfuse Keycloak client; do not create `spoke-*/keycloak-clients`. |
