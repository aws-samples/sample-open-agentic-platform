...
template: {
	...
	outputs: {
		...
		if parameter.registerWithGateway {
			...
			// TODO(agent-identity): Permissive JWT so agents can call MCP without a bearer token.
			// Replace with Strict + OAuth2 Token Exchange (RFC 8693) once per-agent Keycloak
			// client_credentials flow is implemented. See agent-gateway/values.yaml for design.
			mcpAuthPolicy: {
				apiVersion: "agentgateway.dev/v1alpha1"
				kind:       "AgentgatewayPolicy"
				metadata: {
					name:      parameter.name + "-mcp-auth"
					namespace: parameter.namespace
					labels: "app.kubernetes.io/name": parameter.name
				}
				spec: {
					...
					traffic: jwtAuthentication: mode: "Permissive"
				}
			}
		}
